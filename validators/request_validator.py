...
class RequestValidator:
    ...
    def wait_for_request(
                ...
                if json_body:
                    try:
                        ...
                        raise AssertionError(
                            f"Request found for endpoint '{endpoint}' but body is not valid JSON: {e}"
                        )
                    self._validate_json_body(json_body, flow_json)

                if form_urlencoded_body:
                    self._validate_form_urlencoded_body(
                        form_urlencoded_body, flow.request.text
                    )
        ...
        raise TimeoutError(
            f"Request detected for endpoint '{endpoint}' but headers/body did not match expected values"
        )
    ...
    def _validate_form_urlencoded_body(self, expected, actual_text):
        ...
        for key, value in expected.items():
            if key not in actual_dict:
                ...
                raise AssertionError(
                    f"Form field not found: '{key}' (expected value: '{value}').{suggestion}"
                )
